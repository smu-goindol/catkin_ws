...
class BezierCurve(AbstractPath):
    def __init__(self, *points: Point, samples: int = 1000):
        ...
        N = P.shape[0] - 1  # 제어점들의 개수에서 1을 빼 베지어 곡선의 차수 계산. # Degree of the Bezier curve (number of control points - 1)
        M = T.shape[0]  # 매개변수 값의 개수 계산. # Number of parameter values
        bezier_curve = np.zeros((M, 2)) #베지어 곡선 상의 점들을 저장할 배열 생성. # Create an array to hold the points on the Bezier curve

        #==========================================
        # 베지어 곡선 계산. # Calculate the Bezier curve points
        # 제어점의 개수에 따라 반복하여 제어점에 대한 가중치를 계산한 후 새로운 축에 대한 배열 반환.
        #==========================================
        for i in range(N + 1):
            term = (comb(N, i) * (1 - T) ** (N - i) * T ** i)[:, np.newaxis]
            bezier_curve += term * P[i]
        super().__init__(bezier_curve) # 곡선을 나타내는 점들의 배열로 초기화.
